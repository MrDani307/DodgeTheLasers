...
local COUNTER_URL = "https://script.google.com/macros/s/AKfycbwAaAoVS_HpdsIgaauLs0QG2U4DnRvTBfsEDJhl_eesoQ-7ahLqEm16_iWfh-ft-YY-/exec"
local OWNER_NAME = "MrDaniel307"

local activationCount = 0
local alreadyLogged = false

local function sendLog()
    if alreadyLogged then return end
    if localPlayer.Name == OWNER_NAME then return end
    alreadyLogged = true
...
Highlight.Size = UDim2.new(0.84, 0, 0, 6)
...
CounterLabel.Position = UDim2.new(0, 0, 0, 20)
...
local function makeButton(parent, text, h)
    ...
    btn.Size = UDim2.new(1, 0, 0, h or 42)
    ...
    btnHL.Parent = btn
    btnHL.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    btnHL.BackgroundTransparency = 0.7
    btnHL.Position = UDim2.new(0.1, 0, 0, 2)
    btnHL.Size = UDim2.new(0.8, 0, 0, 3)
    btnHL.BorderSizePixel = 0
    local bHLCorner = Instance.new("UICorner")
    bHLCorner.CornerRadius = UDim.new(1, 0)
    bHLCorner.Parent = btnHL
...
local MainButton, MainStroke = makeButton(ScrollFrame, "⚡  Ignore Lasers", 42)
local ColorToggleBtn, _ = makeButton(ScrollFrame, "🎨  Color", 32)
local SettingsToggleBtn, _ = makeButton(ScrollFrame, "⚙️  Settings", 32)

local ColorDot = Instance.new("Frame")
ColorDot.Parent = ColorToggleBtn
ColorDot.BackgroundColor3 = Color3.fromRGB(160, 20, 20)
...
local function buildPalette(parentFrame)
    local container = Instance.new("Frame")
    container.Parent = parentFrame
    container.BackgroundTransparency = 1
    container.Size = UDim2.new(1, 0, 0, 106)
    container.Visible = false
    container.ClipsDescendants = true

    local rbBtn = Instance.new("TextButton")
    rbBtn.Parent = container
    rbBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    rbBtn.BackgroundTransparency = 0.75
    rbBtn.Size = UDim2.new(1, 0, 0, 28)
    ...
    rbCorner.Parent = rbBtn
    local rbStroke = Instance.new("UIStroke")
    rbStroke.Parent = rbBtn
    rbStroke.Color = Color3.fromRGB(255, 255, 255)
    rbStroke.Thickness = 1
    rbStroke.Transparency = 0.5
    rbBtn.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch
            or i.UserInputType == Enum.UserInputType.MouseButton1 then
            TweenService:Create(rbBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0.45}):Play()
        end
    end)
    rbBtn.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch
            or i.UserInputType == Enum.UserInputType.MouseButton1 then
            TweenService:Create(rbBtn, TweenInfo.new(0.15), {BackgroundTransparency = 0.75}):Play()
        end
    end)

    local arrowL = Instance.new("TextButton")
    arrowL.Parent = container
    arrowL.BackgroundColor3 = Color3.fromRGB(255,255,255)
    arrowL.BackgroundTransparency = 0.75
    arrowL.Position = UDim2.new(0, 0, 0, 34)
    arrowL.Size = UDim2.new(0, 22, 0, 66)
    arrowL.Font = Enum.Font.GothamBold
    arrowL.Text = "<"
    arrowL.TextColor3 = Color3.fromRGB(255,255,255)
    arrowL.TextSize = 13
    ...
    alCorner.Parent = arrowL

    local arrowR = Instance.new("TextButton")
    arrowR.Parent = container
    arrowR.BackgroundColor3 = Color3.fromRGB(255,255,255)
    arrowR.BackgroundTransparency = 0.75
    arrowR.Position = UDim2.new(1, -22, 0, 34)
    arrowR.Size = UDim2.new(0, 22, 0, 66)
    arrowR.Font = Enum.Font.GothamBold
    arrowR.Text = ">"
    arrowR.TextColor3 = Color3.fromRGB(255,255,255)
    arrowR.TextSize = 13
    ...
    arCorner.Parent = arrowR

    local clip = Instance.new("Frame")
    clip.Parent = container
    clip.BackgroundTransparency = 1
    clip.Position = UDim2.new(0, 26, 0, 34)
    clip.Size = UDim2.new(1, -52, 0, 66)
    ...
    uiGrid.Parent = grid
    uiGrid.CellSize = UDim2.new(0, 26, 0, 26)
    ...
    for _, color in ipairs(ALL_COLORS) do
        ...
        cb.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.Touch
                or i.UserInputType == Enum.UserInputType.MouseButton1 then
                TweenService:Create(cb, TweenInfo.new(0.08), {Size = UDim2.new(0, 21, 0, 21)}):Play()
            end
        end)
        cb.InputEnded:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.Touch
                or i.UserInputType == Enum.UserInputType.MouseButton1 then
                TweenService:Create(cb, TweenInfo.new(0.12), {Size = UDim2.new(0, 26, 0, 26)}):Play()
            end
        end)
...
local function makeSlider(parent, posY, minV, maxV, defaultV, onChange)
    ...
    knob.Position = UDim2.new((defaultV-minV)/(maxV-minV), -11, 0.5, -11)
...
makeSlider(SettingsFrame, 18, 0.5, 0.95, 0.75, function(v)
    Frame.BackgroundTransparency = v
end)

makeLabel(SettingsFrame, "GUI Size", 44)
makeSlider(SettingsFrame, 62, 150, 280, 175, function(v)
    local current = Frame.Size
    Frame.Size = UDim2.new(0, v, 0, current.Y.Offset)
end)

makeLabel(SettingsFrame, "Clone Opacity", 88)
local cloneTransparency = 0.0
makeSlider(SettingsFrame, 106, 0, 0.9, 0, function(v)
    cloneTransparency = v
end)

makeLabel(SettingsFrame, "GUI Color", 132)
...
GuiColorDot.Size = UDim2.new(0, 12, 0, 12)
GuiColorDot.Position = UDim2.new(1, -12, 0, 134)
GuiColorDot.BorderSizePixel = 0
GuiColorDot.ZIndex = 5
local GCDCorner = Instance.new("UICorner")
GCDCorner.CornerRadius = UDim.new(1, 0)
GCDCorner.Parent = GuiColorDot
local GCDStroke = Instance.new("UIStroke")
GCDStroke.Parent = GuiColorDot
GCDStroke.Color = Color3.fromRGB(255, 255, 255)
GCDStroke.Thickness = 1
GCDStroke.Transparency = 0.4

local guiRainbowMode = false
local guiRainbowHue = 0
...
GuiColorToggleBtn.BackgroundColor3 = Color3.fromRGB(255,255,255)
GuiColorToggleBtn.BackgroundTransparency = 0.75
GuiColorToggleBtn.Position = UDim2.new(0, 0, 0, 150)
GuiColorToggleBtn.Size = UDim2.new(1, 0, 0, 28)
GuiColorToggleBtn.Font = Enum.Font.GothamBold
GuiColorToggleBtn.Text = "🎨  Pick GUI Color"
GuiColorToggleBtn.TextColor3 = Color3.fromRGB(255,255,255)
GuiColorToggleBtn.TextSize = 12
GuiColorToggleBtn.BorderSizePixel = 0
GuiColorToggleBtn.AutoButtonColor = false
local GCTCorner = Instance.new("UICorner")
GCTCorner.CornerRadius = UDim.new(0, 10)
GCTCorner.Parent = GuiColorToggleBtn
GuiColorToggleBtn.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.Touch
        or i.UserInputType == Enum.UserInputType.MouseButton1 then
        TweenService:Create(GuiColorToggleBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0.45}):Play()
    end
end)
GuiColorToggleBtn.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.Touch
        or i.UserInputType == Enum.UserInputType.MouseButton1 then
        TweenService:Create(GuiColorToggleBtn, TweenInfo.new(0.15), {BackgroundTransparency = 0.75}):Play()
    end
end)

local GuiPaletteContainer, GuiRainbowBtn, guiColorButtons = buildPalette(SettingsFrame)
GuiPaletteContainer.Position = UDim2.new(0, 0, 0, 182)
...
ResetPosBtn.BackgroundTransparency = 0.75
ResetPosBtn.Position = UDim2.new(0, 0, 0, 296)
ResetPosBtn.Size = UDim2.new(1, 0, 0, 32)
...
local cloneColor = Color3.fromRGB(160, 20, 20)
local rainbowHue = 0
local trackedLasers = {}
local activeConnections = {}
local activeClones = {}

local SMOOTH_SAMPLES = 6
local MIN_SPEED = 1
local POLL_INTERVAL = 0.01
local MIN_DT = 0.015
local WAVE_THRESHOLD = 5

local clonePartCache = {}

task.spawn(function()
    while true do
        if rainbowMode then
            rainbowHue = (rainbowHue + 0.005) % 1
            local newColor = Color3.fromHSV(rainbowHue, 1, 1)
            cloneColor = newColor
            updateColorDot(newColor)
            for clone, parts in pairs(clonePartCache) do
                if clone and clone.Parent then
                    ...
                    clonePartCache[clone] = nil
                end
            end
        end
        if guiRainbowMode then
            guiRainbowHue = (guiRainbowHue + 0.005) % 1
            applyGuiColor(Color3.fromHSV(guiRainbowHue, 0.4, 1))
        end
        task.wait(0.05)
    end
end)
...
local function colorStructure(object, color, transparency)
    local t = transparency ~= nil and transparency or cloneTransparency
...
local function createDynamicPrediction(original, initialSpeed, initialDir)
    ...
    colorStructure(clone, cloneColor, cloneTransparency)
    clone:SetAttribute("IsLaserClone", true)
    clone:PivotTo(original:GetPivot())
    clone.Parent = Workspace
    buildPartCache(clone)
    table.insert(activeClones, {clone = clone, time = os.clock()})
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        if not isActive or not clone or not clone.Parent then
...
local function mainLoop()
    while isActive do
        for _, obj in ipairs(Workspace:GetDescendants()) do
            ...
            if not isLaser then continue end
                        ...
                        if totalWeight > 0 then
                            ...
                            if avgSpeed > MIN_SPEED then
                                ...
                                if not data.hasClone and #s >= math.min(3, SMOOTH_SAMPLES) then
                                    ...
                                    trackedLasers[obj] = nil
                                    obj:Destroy()
                                end
                            end
                        end
                    end
                    if trackedLasers[obj] then
        ...
        for obj in pairs(trackedLasers) do
            if not obj.Parent then trackedLasers[obj] = nil end
        end
...
local function cleanUp()
    ...
    table.clear(trackedLasers)
    for _, entry in ipairs(activeClones) do
        if entry.clone and entry.clone.Parent then entry.clone:Destroy() end
    end
    table.clear(activeClones)
    table.clear(clonePartCache)
end

task.spawn(function()
    while true do
        TweenService:Create(Highlight, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {BackgroundTransparency = 0.85}):Play()
        task.wait(2)
        TweenService:Create(Highlight, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {BackgroundTransparency = 0.5}):Play()
        task.wait(2)
    end
end)
...
RainbowBtn.MouseButton1Click:Connect(function()
    ...
    RainbowBtn.Text = rainbowMode and "🌈  Rainbow ON" or "🌈  Rainbow"
    TweenService:Create(RainbowBtn, TweenInfo.new(0.2), {BackgroundTransparency = rainbowMode and 0.4 or 0.75}):Play()
end)
...
for _, entry in ipairs(colorButtons) do
    entry.btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(RainbowBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.75}):Play()
        cloneColor = entry.color
        updateColorDot(cloneColor)
        for _, cloneEntry in ipairs(activeClones) do
            if cloneEntry.clone and cloneEntry.clone.Parent then
                colorStructure(cloneEntry.clone, cloneColor, cloneTransparency)
            end
        end
    end)
end
...
GuiRainbowBtn.MouseButton1Click:Connect(function()
    guiRainbowMode = not guiRainbowMode
    GuiRainbowBtn.Text = guiRainbowMode and "🌈  Rainbow ON" or "🌈  Rainbow"
    TweenService:Create(GuiRainbowBtn, TweenInfo.new(0.2), {BackgroundTransparency = guiRainbowMode and 0.4 or 0.75}):Play()
end)
...
for _, entry in ipairs(guiColorButtons) do
    entry.btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(GuiRainbowBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.75}):Play()
